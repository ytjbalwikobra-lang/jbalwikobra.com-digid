-- Complete schema fix for production - creates all missing relations and data
-- Run this in Supabase Dashboard SQL Editor

-- Enable UUID extension if not exists
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 1. Create tiers table if it doesn't exist
CREATE TABLE IF NOT EXISTS tiers (
    ...
    price_range_max INTEGER NOT NULL DEFAULT 10000000, -- Set default value to avoid null constraint issues
...
-- 2. Create game_titles table if it doesn't exist
...
-- 3. Add missing columns to products table if they don't exist
DO $$
BEGIN
    -- Add tier_id column
    ...
    -- Add account_level column if missing
    ...
    -- Add account_details column if missing
    ...
    -- Add images array column if missing
...
-- 5. Insert tier data (with conflict handling)
...
-- 6. Insert game titles data (with conflict handling)
...
-- 7. Update existing products to have proper foreign key relationships
...
-- 11. Create or replace the optimized view
...
-- 12. Insert some sample products if products table is empty
...
SELECT 
    'Akun PUBG Conqueror Asia',
    'Akun PUBG Mobile rank Conqueror server Asia. KD ratio 4.5, tier rewards lengkap.',
    1800000,
    2200000,
    'https://images.unsplash.com/photo-1542751371-adc38448a05e?w=400',
    ARRAY['https://images.unsplash.com/photo-1542751371-adc38448a05e?w=400'],
    (SELECT id FROM tiers WHERE slug = 'pelajar'),
    (SELECT id FROM game_titles WHERE slug = 'pubg-mobile'),
    'Conqueror',
    'KD 4.5, All season rewards, Mythic weapons',
    true,
    8,
    false
WHERE (SELECT COUNT(*) FROM products) = 1;

-- Add some rental options for the sample products
...
SELECT 
    ...
    '3 Hari',
    400000,
    'Akses 3x24 jam + bonus coaching'
FROM products p 
WHERE p.name = 'Akun ML Mythic Glory Premium'
AND NOT EXISTS (SELECT 1 FROM rental_options WHERE product_id = p.id AND duration = '3 Hari');
